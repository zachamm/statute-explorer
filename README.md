...
It currently works out of the box for **any federal statute** (laws-lois.justice.gc.ca)
and **any Ontario statute** (ontario.ca/laws) — see [Importing a statute](#importing-a-statute)
below for how to add one.
...
`laws-lois.justice.gc.ca/eng/XML/<chapterCode>.xml`. This is a real, clean,
well-formed XML schema — `Statute > Body > (Heading | Section)`, with
...
citations marked with `<XRefExternal>`. Because every federal Act uses this
exact schema, `parse.js` needed zero Act-specific code — the chapter code is
the only input, and it's been verified end-to-end against three
structurally different Acts ranging from 64 to 1,603 sections (see
`docs/SCHEMA.md` for the full element mapping).
...
`class="subsection"`, `class="definition"`, `class="partnum"`, ...) rather
than by distinct tags, and the HTML itself isn't well-formed (Word's
converter leaves unclosed/mis-nested `<span>` tags — e.g. a citation note
gets wrapped in three separately-nested `<span class="citation">` tags,
one per word). `parse-ontario.js` uses `cheerio` (an HTML5-tolerant parser)
to recover a sane DOM from that, then walks the flat `<p>` stream
classifying each paragraph by its class name. This was reverse-engineered
against a real downloaded Act, not documented anywhere by Ontario, so it's
inherently more heuristic than the federal adapter — see **Known
limitations** below.
...
- `parser/lib/text-runs.js` — turns plain text into the linked
  ...
  the same style (which covers most of Canada — the convention is
  nationwide, not jurisdiction-specific).
- `parser/lib/write-statute.js` — writes the output JSON, updates
...
(heuristic, HTML-by-CSS-class) are two different answers to that same
problem and are a reasonable pair of references for how different a real
adapter can end up looking.
...
- **Schedules are skipped** in both adapters (federal `<Schedule>` elements;
  ...
  v1, same as the original federal-only design.
- **Ontario's schedule-boundary detection is a heuristic**, not a structural
  ...
  schedule in some other Ontario Act could in principle leak stray content
  into the tree. If a parsed Ontario Act ever shows a obviously-wrong
  section in the tree, this is the first place to check.
- **Defined-term linking is whole-word substring matching**, case-
  ...
  proper noun that happens to contain a defined word (e.g. "**Ambulance**
  Act" gets "Ambulance" linked to the defined term "ambulance" even though
  it's naming a different Act, not using the term). This is a known,
  accepted imprecision of the design, not a bug to chase — see the "Why
  this shape" section of `docs/SCHEMA.md`.
